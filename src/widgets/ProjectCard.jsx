...
const ProjectCard = ({ name, duration, methodology, content, img, technologies, href }) => (
    <div className="mb-12 lg:mb-0 feedback-card p-7 rounded-lg border border-gray-700 shadow-xl">
        <div className="relative sm:mb-6 mb-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-black/20 bg-[50%]">
            <img src={`/images/${img}`} className="w-full lg:h-[270px]" alt={"project img"} />
        </div>
        ...
        <div className={`flex flex-wrap mb-2 transition-all duration-250 ease-in-out`}>
            {technologies && technologies.map(index => (
                <div key={index} className={`flex p-2 mr-5 mb-3 small-card rounded-md w-[40px] h-[40px] ${styles.flexCenter}`}>
        ...
        <a href={href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-3 py-2 text-sm font-medium font-poppins text-center text-primary bg-blue-gradient rounded-lg">
            View More
